...
func verifyFullSRouterContext(t *testing.T, ctx context.Context, testName string) {
	...
	copiedBuildID, ok := GetBuildID(ctx)
	...
	copiedConfigID, ok := GetConfigID(ctx)
	...
	copiedUserAgent, ok := GetUserAgent(ctx)
	...
	copiedTx, ok := GetTransaction(ctx)
	if !ok || copiedTx == nil {
	...
	copiedOrigin, copiedCreds, ok := GetCORSInfo(ctx)
	...
	copiedHeaders, ok := GetCORSRequestedHeaders(ctx)
	...
	copiedHandlerErr, ok := GetHandlerError(ctx)
...
func TestCopySRouterContext_Independence(t *testing.T) {
	...
	// Verify independence
	srcFlag, ok := GetFlag(srcCtx, "shared-flag")
	if !ok || srcFlag {
		t.Errorf("Source context flag should be false, got %v (ok: %v)", srcFlag, ok)
	}

	copiedFlag, ok := GetFlag(copiedCtx, "shared-flag")
	if !ok || !copiedFlag {
...
func TestCopySRouterContextOverlay(t *testing.T) {
	t.Run("FullCopy_DestinationHasContext", func(t *testing.T) {
		...
		_, ok := GetFlag(copiedCtx, "dst-flag")
		if ok {
	...
	t.Run("Independence", func(t *testing.T) {
		...
		// Verify independence
		srcFlag, _ := GetFlag(srcCtx, "shared-flag")
		copiedFlag, _ := GetFlag(copiedCtx, "shared-flag")
